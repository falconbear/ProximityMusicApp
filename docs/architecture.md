...
## ルーティング設計の判断
### GoRouter採用の根拠
- **URI ベースルーティング**: 宣言的で状態管理しやすい
- **Deep Link 対応**: 外部からの画面直接起動 (`proximity-music://player`)
- **Web 対応**: 同一コードでWeb版も動作
- **Flutter 3.0+ 推奨**: 公式推奨パターンでメンテナンス性が高い
- **テスト容易性**: 特定画面への直接遷移が簡単

### 従来Navigator比較
- **従来方式**: `Navigator.push/pop` (スタック管理)
- **GoRouter**: 宣言的ルート定義 + 状態復元対応
- **選択理由**: スマホアプリでもURI形式の恩恵が大きい（Deep Link、状態復元、拡張性）
